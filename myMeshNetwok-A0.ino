...
//  JsonObject& sensorbroadcast = jsonBuffer.createObject();
//  sensormsg["Node-id"] = mesh.getNodeId();
//  sensormsg["sensor-value"] = sensorvalue;
//  sensormsg["Temp"] = Temperature;
//  sensormsg["Hum"] = Humidity;
...
void loop() {
  ...
  if (serialCount == 1000) {
    ssValue=analogRead(analogInPin);
...
void sendMessage() {
  ...
  ////////////////////////////////在此後加入傳感器上傳的部份
  float sensorValue = ((ssValue/1024)*3.3*2-3.7)/0.5; //手動定義，應由機器讀取傳感器而得
  DynamicJsonBuffer jsonBuffer;
  JsonObject& sensormsg = jsonBuffer.createObject();
  JsonObject& sensorbroadcast = jsonBuffer.createObject();
  ...
  sensormsg["Hum"] = Humidity;
  sensormsg["node-id"] = mesh.getNodeId();
  sensormsg["sensor-value"] = sensorValue; //sensormsg樣式如{"node-id":33333,"sensor-value":12.55}
  sensormsg["rssi"] = rssi;
  sensorbroadcast["sensorBroadcast"] = sensormsg; //sensorbroadcast={"sensorBroadcast":sensormsg樣式}
  String sstr;
  sensorbroadcast.printTo(sstr);
  Serial.println(sstr);//把sensorbroadcast字串化，送wifi廣播
  mesh.sendBroadcast(sstr);
  /////////////////////////////////////以上新增部份
}
...
void readSerial()
{
  ...
  if (Serial.available())
  {
    ...
    Serial.println("TSK Serial received:" + data);
...
void receivedCallback(uint32_t from, String &msg)
{
  ...
  if (message.success())
  {
    ...
    /////////////以下新增對傳廣器廣播訊息處理部份
    else if (message.containsKey("sensorBroadcast")) {
      JsonObject& sensorParams = message["sensorBroadcast"];
      String sstr;
      sensorParams.printTo(sstr);
      Serial.println(sstr);   //此處將wifi廣播而來的訊息轉送串列口
    }
